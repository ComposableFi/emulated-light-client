...
/// Data to hash by the program.
pub enum Data {
    /// Buffer of given length filled with zero bytes.
    Zeros(u32),
    /// Data of given account.
    Account(Pubkey),
}

/// Prints usage information.
fn usage(argv0: &str, full: bool) {
    eprintln!("usage: {argv0} [<switch> ...]");
    if !full {
    ...
    eprintln!(concat!(
        ...
        "    -b --built-in         Use built-in SHA2 syscall\n",
        "    -l --length=<len>     Hash <len>-byte long buffer\n",
        "    -a --account=<acc>    Hash data of given account\n",
        "\n",
...
fn parse_impl(prog: &mut Prog) -> Result<Opts, bool> {
    ...
    while let Some(arg) = prog.next() {
        ...
        {
            program_id = Some(id);
        } else if arg == "-b" || arg == "--built-in" {
            builtin = true;
        } else if let Some(len) =
            prog.parse_flag(&arg, "-l", "--length", u32::from_str)?
        {
            data = Some(Data::Zeros(len))
        } else if let Some(account) =
            prog.parse_flag(&arg, "-a", "--account", Pubkey::from_str)?
        {
            data = Some(Data::Account(account))
        } else {
            eprintln!("{prog}: {arg}: unknown argument");
            return Err(false);
        }
    }
    ...
    let data = data.ok_or_else(|| {
        eprintln!("{prog}: missing --length or --account");
        false
    })?;
